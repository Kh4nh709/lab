<?php
    ...
    if(isset($_POST['username']) && isset($_POST['password'])){
        ...
        $result = get_username_ch_1($username,$password);
        if($result){
            $flag = get_flag_by_id(1);
        }else{
...
<body>
    <div class="login-container">
        ...
        <div style="display: flex; justify-content: space-between; margin-top: 24px;">
            <!-- Previous Challenge button: only show if not challenge 1 -->
            <!-- Since this is ch_1.php, do not show previous button -->
            <div></div>
            <!-- Next Challenge button: only show if not challenge 10 -->
            <a href="ch_2.php" style="display:inline-block; padding:10px 16px; background:#007bff; color:#fff; border-radius:4px; text-decoration:none; font-size:1em; text-align:center;">Next Challenge</a>
        </div>
    </div>
</body>
</html>
